...
class AddressAction extends Controller
{
    ...
    public function myRedbag(){
        ...
        if($type==1){
            ...
            $list = $model->table('redbag as r')->join('left join customer as c on r.user_id = c.user_id left join redbag_get as rg on r.id = rg.redbag_id left join user as u on r.user_id=u.id')->fields('r.id,r.amount,c.real_name,rg.amount as get_money,rg.get_date,u.avatar')->where("r.status=1 and rg.get_user_id=".$user_id)->order('r.id desc')->findPage($page, 10);
        }
