...
class UcenterAction extends Controller {
    ...
    public function login() {
        ...
        $obj = $model->join("left join customer as cu on us.id = cu.user_id left join oauth_user as ou on us.id = ou.user_id")->fields("us.*,cu.group_id,cu.user_id,cu.login_time,cu.mobile")->where("us.name='$account' or cu.mobile='$account' or us.email='$account'")->find();
        if ($obj) {
            if ($obj['status'] == 1) {
