...
class UcenterAction extends Controller {
    ...
    public function login() {
        ...
        $obj = $model->join("left join customer as cu on us.id = cu.user_id")->fields("us.*,cu.group_id,cu.user_id,cu.login_time,cu.mobile")->where("us.name='$account' or cu.mobile='$account' or us.email='$account' and cu.status=1")->find();
        if ($obj) {
            if ($obj['status'] == 1) {
    ...
    public function balance_log() {
        ...
        switch ($type) {
            case 'in':
                $where = ' and type in(1,2,4,5,6,7,8,9,10,12,13)';
                break;
            case 'out':
                $where = ' and type in(0,3,11)';
                break;
            case 'all':
                break;
            default:
    ...
    public function offlinebalance_log() {
        ...
        switch ($type) {
            case 'in':
                $where = ' and type in(8,13)';
                break;
            case 'out':
                $where = ' and type = 11';
                break;
            case 'all':
                $where = ' and type in(8,11,13)';
                break;
            default:
                break;
        }
   ...
   //获得收藏列表
    ...
    public function bankcardList(){
        ...
        $newlist = $this->model->table('bankcard')->where('user_id='.$this->user['id'])->findAll();
        if($newlist){
