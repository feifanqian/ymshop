...
class ActiveController extends Controller
{
    ...
    public function recruit() {
        ...
        if($user_id) {
            ...
            $list = $this->model->table("invite as i")->fields("FROM_UNIXTIME(i.createtime) as create_time,u.nickname,u.avatar,cu.real_name")->join("left join user as u on i.invite_user_id = u.id LEFT JOIN customer AS cu ON i.invite_user_id=cu.user_id")->where("i.user_id = " . $this->user['id']." and i.from=active")->limit(4)->findAll();
        } else {
            $invite_num = 0;
            $list = [];
        }
