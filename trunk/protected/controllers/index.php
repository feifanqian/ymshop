...
class IndexController extends Controller {
    ...
    public function huadian(){
        ...
        $result1 =$goods 
                ->where("is_online =0 and is_huabipay = 1 and huabipay_sort >0")->fields("id,name,img,sell_price,market_price,store_nums,huabipay_set")
                ->order('huabipay_sort desc')
                ->limit("0,20")
                ->findAll();
            ...
            foreach ($result1 as $k=>$v){
                if($v['huabipay_set']!=""){
                    ...
                    if(!is_array($set)){
                        ...
                        $result1[$k]['huabipay_amount'] = $huabipay_amount;
                        $result1[$k]['still_pay'] = $stillpay;
                    }
                }else{
                    unset($result1[$k]);
                    break;
        ...
        if(isset($result2['data'])&&!empty($result2['data'])){
            foreach ($result2['data'] as $k=>$v){
               if($v['huabipay_set']!=""){
                    ...
                    if(!is_array($set)){
                        ...
                        $result2['data'][$k]['huabipay_amount'] = $huabipay_amount;
                        $result2['data'][$k]['still_pay'] = $stillpay;
                    }
                }else{
                    unset($result2['data'][$k]);
                    break;
          ...
          $this->assign('recommend',$result1);
          $this->assign('normal',$result2);
        }else{
        $result3 = $goods
        ->where("is_online =0 and is_huabipay = 1")->fields("id,name,img,sell_price,market_price,store_nums,huabipay_set")
        ->order("huabipay_sort desc")
        ->findPage($page,10,4);
        if(isset($result3['data'])&&!empty($result3['data'])){
            foreach ($result3['data'] as $k=>$v){
               if($v['huabipay_set']!=""){
                    $set = unserialize($v['huabipay_set']);
                    if(!is_array($set)){
                        unset($result3['data'][$k]);
                        break; 
                    }else{
                           //取第一条设置
                           usort($set, function($a,$b){
                        ...
                        $result3['data'][$k]['huabipay_amount'] = $huabipay_amount;
                        $result3['data'][$k]['still_pay'] = $stillpay;
                    }
                }else{
                    unset($result3['data'][$k]);
                    break;
          ...
          $this->assign('data',$result3);
        }
        $this->assign('seo_title', '会员专区');
        $this->assign('seo_keywords', '会员专区，优惠，兑换');
        $this->redirect();
    }
