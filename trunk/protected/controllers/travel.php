...
class TravelController extends Controller
{
    ...
    public function order_list()
    {
        if($this->user['id']) {
            ...
            $list = $this->model->table('travel_order as t')->fields('t.id,t.order_no,tw.name,tw.city,tw.desc,tw.order_amount,tw.img')->join('left join travel_way as tw on t.way_id=tw.id')->where('t.user_id='.$this->user['id'])->findPage($page,10);
            $this->assign('list',$list); 
            $this->redirect();
        } else {
