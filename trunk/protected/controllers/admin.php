...
class AdminController extends Controller {
    ...
    public function init() {
        //只允许从mana进入
        if (stripos($_SERVER['REQUEST_URI'], 'mana') === FALSE) {
//            Tiny::Msg("system", "您所请求的页面未找到");
//            exit;
        }
        $menu = new Menu();
    ...
    public function noRight() {
        ...
        if ($this->is_ajax_request()) {
            echo JSON::encode(array('status' => 'fail', 'msg' => '123'));
        } else {
    ...
    public function check() {
        ...
        //if ($code != strtolower(Req::args($this->captchaKey))) {
            ...
            SMS::getInstance()->flushCode($mobile['mobile']);
    ...
    function class_config_edit() {
        $class_name = Req::args('class_name');
        $model = new Model("class_config");
        $data = $model->where("class_name='{$class_name}'")->find();
        $this->assign($data);
        $this->redirect('class_config_edit');
    }
    ...
    function theme_list() {
            ...
            if ($dh = opendir($theme_path)) {
                while (($file = readdir($dh)) !== false) {
                    if ($file != '.' && $file != '..' && is_dir($theme_path . $file)) {
                        $info = include($theme_path . $file . "/info.php");
                        if (isset($info['type']) && $info['type'] == 'mobile')
                            $themes['mobile'][] = array('file' => $file, 'info' => $info);
                        else
                            $themes['pc'][] = array('file' => $file, 'info' => $info);
                    }
                }
                closedir($dh);
            }
        ...
        if (isset($config['theme']))
            $theme_pc = $config['theme'];
        else
            $theme_pc = 'default';
        if (isset($config['themes_mobile']))
            $themes_mobile = $config['themes_mobile'];
        else
            $themes_mobile = 'default';
    ...
    public function payment_list() {
        ...
        $where = isset($_GET['client_type']) ? "pa.client_type='{$_GET['client_type']}'" : "1=1";
        $list = $model->join("left join pay_plugin as pi on pa.plugin_id = pi.id")->fields("pa.id,pa.plugin_id,pa.pay_name,pa.client_type,pa.status,pi.description,pi.logo,pi.class_name")->where($where)->order('pa.sort desc')->findAll();
        $typearr = array(0 => 'PC', 1 => '移动', 2 => '微信', 3 => '安卓', 4 => 'iOS');
        foreach ($list as $k => &$v) {
            $v['client'] = isset($typearr[$v['client_type']]) ? $typearr[$v['client_type']] : 'PC';
        }
        unset($v);
    ...
    public function payment_validator() {
        ...
        if ($fee_type == 2)
            Req::args("pay_fee", Req::args("pay_fee_fix"));
    ...
    public function notice_template_validator() {
        ...
        if (!$right && $status == 1)
            return array('msg' => '你使用的版本不支持' . $style_name . '提醒功能，请不要开启');
    }
    ...
    public function manager_save() {
        ...
        if ($id) {
            ...
            if ($user) {
                if ($roles) {
                    $managerModel->data(array('roles' => $roles,'mobile'=>$mobile))->where("id=$id")->update();
                    Log::op($this->manager['id'], '修改管理员角色', '修改管理员【' . $user['name'] . '】的角色为【' . $roles_name . '】手机号码为【'.$mobile.'】！');
                }
            }
        } else {
            ...
            if ($user) {
                ...
                $managerModel->data(array('name' => $name, 'password' => $password, 'roles' => $roles, 'validcode' => $validcode,'mobile'=>$mobile))->insert();
    ...
    public function manager_del() {
        ...
        if ($manager)
            Log::op($this->manager['id'], '删除管理员', '删除管理员【' . $manager['name'] . '】！');
        $model->where("id=$id")->delete();
        $this->redirect("manager_list");
    }
    ...
    function photoshop() {
        ...
        $options = array(
            'cdnurl' => $upyun['upyun_cdnurl'],
            'bucket' => $upyun['upyun_bucket'],
            'save-key' => $upyun['upyun_save-key'],
            'formkey' => $upyun['upyun_formkey'],
        ...
        $signature = md5($policy . '&' . $options['formkey']);
        ...
        if ($file['error'] == 4) {
            ...
            if (!$img) {
                ...
                if ($info[0]['status'] == 1) {
                    ...
                    $type = Req::args("type") == null ? "unknown" : Req::args('type');
                    $gallery->data(array('key' => $key, 'type' => $type, 'url' => $url))->save();
    ...
    function photoshop_uploadifive() {
        $code = -1;
        // Set the allowed file extensions
        $fileTypes = array('jpg', 'jpeg', 'gif', 'png'); // Allowed file extensions
        $verifyToken = md5('Ey7jpbuC' . $_POST['timestamp']);
        ...
        if (!empty($_FILES) && $_POST['token'] == $verifyToken) {
            ...
            if (in_array(strtolower($fileParts['extension']), $fileTypes)) {
                ...
                $upfile_path = Tiny::getPath("uploads");
                $upfile_url = preg_replace("|^" . APP_URL . "|", '', Tiny::getPath("uploads_url"));
                $upfile = new UploadFile('Filedata', $upfile_path, '10m');
                $upfile->save();
                $info = $upfile->getInfo();
                $result = array();
                if ($info[0]['status'] == 1) {
                    ...
                    $message = array('error', $info[0]['msg']);
                    $message = $msg;
                }
            } else {
                $message = '不允许上传的文件类型';
    ...
    public function send_email_test() {
        ...
        if (Validator::email($test_email)) {
            $mail = new Mail();
            try {
                $flag = $mail->send_email($test_email, '邮箱验证测试', "<div><h3>亲爱的用户您好:</h3><p style='text-indent:2em;'>当您收到这封邮件时，说明您的信息配制正确，邮件系统可以正常工作。</p></div>");
