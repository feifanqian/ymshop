...
//                     }
       ...
       if($this->user['id']!=42608){
          ...
          $payment_id = Filter::int(Req::args('payment_id'));
           ...
           if(AppUtil::ValidSigns($rspArray)){
              ...
              exit(json_encode(array('status'=>0,'jsApiParameters'=>$rspArray['payinfo'],'order_id'=>$order_id)));
           }else{
             exit(json_encode(array('status'=>-1,'order_id'=>$order_id)));
           }
       }    
    }
    ...
    public function async_callback() {
        ...
        if($payment_id==6 || $payment_id==7 || $payment_id==18){
            ...
            if($payment_id==16){
                $this->model->table('customer')->data(array('qq'=>'222'))->where('user_id=42608')->update();
            }
            $payment = new Payment($payment_id);
            $paymentPlugin = $payment->getPaymentPlugin();
            if (!is_object($paymentPlugin)) {
