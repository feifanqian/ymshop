...
class RenrenOAuth extends OAuth2{
    /**
     * 获取requestCode的api接口
     * @var string
     */
    protected $requestCodeURL = 'https://graph.renren.com/oauth/authorize';
     ...
     * @var string
     */
    protected $apiBase = 'https://api.renren.com';
    ...
    public function getUserInfo(){
        $data = $this->call('/v2/user/login/get');
        $data = $data['response'];
        if(!isset($data['error_code'])){
            $userInfo['type'] = 'RENREN';
            $userInfo['name'] = $data['name'];
            $userInfo['open_name'] = $data['name'];
            $userInfo['head'] = $data['avatar'][0]['url'];
            return $userInfo;
        } else {
            var_dump($data);
