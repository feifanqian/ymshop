...
class Log {
    ...
    public static function incomeLog($amount , $role_type, $role_id , $record_id ,$type ,$note=""){
        ...
        if($role_type==1||$role_type==2){
            ...
            $result = $model->table("customer")->data(array("valid_income"=>"`valid_income`+({$data['valid_income_change']})","frezze_income"=>"`frezze_income`+({$data['frezze_income_change']})","settled_income"=>"`settled_income`+({$data['settled_income_change']})","balance"=>"`balance`+({$data['frezze_income_change']})"))
