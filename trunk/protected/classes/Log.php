...
class Log {
    ...
    public static function balance($amount, $user_id, $order_no='', $note = '', $type = 0, $admin_id = 0) {
        //事件类型: 0:购物下单 1:用户充值 2:管理员充值 3:余额提现 4:管理员退款 5：佣金获取 6:推广收益 7：商城分红 8:下级会员线下消费提成 9:商家余额转入 10:推广收益提现到余额 11:商家余额提现 12:提现回退到余额 13:提现回退到商家余额 14:抢红包所得 15:红包退回
        $model = new Model();
    ...
    public static function pointcoin_log($amount, $user_id, $order_no='', $note = '', $type = 0, $admin_id = 0) {
        //事件类型: 0:购物下单 1:套餐充值 2:系统退回 3：管理员充值 4：管理员退款 5:代理商入驻赠送 6:代理商推荐奖励 7：专区销售积分奖励 8:经营商入驻赠送 9:购买商品赠送 10:每日签到赠送 11：商城分红 12：订单作废，积分退回
        $model = new Model();
        if($type==0){
            $amount = 0 - abs($amount);
        }
