...
/**
 * @class pay_wxpayqrcode
 * @brief 微信支付(扫码支付)
 */
class pay_wxpayqrcode extends PaymentPlugin {

    //支付插件名称
    public $name = '微信支付(扫码支付)';
    public $needSubmit = TRUE;

    //提交地址
    public function submitUrl() {
        return '/payment/pay_wxpayqrcode_submit/';
    }
    ...
    public function packData($payment) {
        ...
        $input->SetOut_trade_no(substr(time(), 6) . '_' . $order_no);
        ...
        $return['out_trade_no'] = substr(time(), 4) . "_" . $payment['M_OrderNO'];
	...
	$result = WxPayApi::refund($input);
        ...
        if($result['return_code']=="SUCCESS" && $result['result_code']=="SUCCESS"){
           ...
           file_put_contents("wxqrcode.txt", "微信退款失败：\r\n".json_encode($result)."\r\n__________________________",FILE_APPEND);
           $error = isset($result['return_msg']) ? $result['return_msg']:"未知";
           $refund->data(array("error_record"=>$error.'['.date("Y-m-d H:i:s").']'))->where("id =$refund_id")->update();
           echo json_encode(array('status' => 'fail', 'msg' => '[微信]退款操作失败，原因：'.$error));
           exit();
        }
    }
    /*
