...
(function ($) {
    $.fn.slide = function (options) {
        ...
        var $li = $box.find(".slideboxli");
        var timer = null;
        var num = 0;

        $this.append(ool);
        $box.find(".slideboxli").each(function (i) {
        ...
        $box.find(".slideboxli").eq(0).show(0);
        ...
        function auto() {
            num < $box.find(".slideboxli").length - 1 ? num++ : num = 0;
            run();
        }
        function run() {
            ...
            switch (opts.affect) {
                case 1:
                    ...
                    break;
                case 2:
                    $box.find(".slideboxli").css({"position": "absolute"});
                    $box.find(".slideboxli").stop(false, true).fadeOut(opts.speed).eq(num).slideDown(opts.speed);
                    break;
                case 3:
                ...
                case 4:
                    $box.find(".slideboxli").css({"position": "absolute"});
                    $box.find(".slideboxli").stop(false, true).fadeOut(opts.speed).eq(num).fadeIn(opts.speed);
                    break;
            }
        }
