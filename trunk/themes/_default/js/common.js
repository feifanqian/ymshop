...
function tabs_init() {
    $(".tab:not([initdisabled])").each(function (j) {
        var that = $(this);
        that.attr("index", 0);
                    ...
                    if (o.content) {
                        str += o.content.replace(/{\s*([^}]+)\s*}/ig, function (s0, s1) {
                            ...
                            for(var j in data[i]['photos']){
                                img_str+='<img src="http://img.buy-d.cn'+data[i]['photos'][j]+'!/fwfh/100x100" onclick="showImg(this);" layer-src="http://img.buy-d.cn'+data[i]['photos'][j]+'" alt="'+data[i]['content']+'">';
                            }
                            return img_str;
                            }
                            if (tem.length == 2) {
...
function updateCart(data) {
    ...
    for (var i in data) {
        ...
        card_items += '<div class="cart-item" id="' + i + '"><div class="pic"><img src="' + siteUrl + data[i]['img'] + '" width="50" height="50"></div><div class="spec">' + spec_str + '</div><div class="num">' + data[i]['num'] + '</div><div class="price">' + (data[i]['amount']) + '</div><a class="icon-close-16 ie6png" productid="' + data[i]['id'] + '"></a></div>';
    }
...
$(function () {
    ...
    $("#shopping-cart").on("click", ".icon-close-16", function () {
        var btn_close = $(this);
        $.post(Tiny.url.cart_del_url, {id: btn_close.attr("productid")}, function () {
