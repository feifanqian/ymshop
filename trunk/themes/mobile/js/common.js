    ...
    app.initPageSwiper = function (pageContainer) {
        ...
        swipers.each(function () {
            ...
            if (swiper.data('swiper')) {
                params = $.parseJSON(swiper.data('swiper').replace(/\'/g, '"'));
            } else {
...
if (!String.prototype.format) {
    String.prototype.format = function () {
        ...
        return this.replace(/(\{|\(|\[)(\d+)(\}|\)|\])/g, function (match, m0, m1, m2) {
            tag = m0 + m2;
            if (tag == '()' || tag == '{}' || tag == '[]')
                return typeof args[m1] != 'undefined' ? args[m1] : (m0 + m1 + m2);
        });
    };
}
function url(str) {
...
function tabs_init() {
    $(".tab").each(function (j) {
        var that = $(this);
        that.attr("index", 0);
        var total = $(">.tab-head > *", that).size();
        var width = (100 / total) + "%";
        $(">.tab-head > *", that).css({width: width});
...
function tools_submit(obj) {
    ...
    if (obj != undefined) {
        ...
        if (form.attr('method') == 'get') {
            var pattern = /(\w+)=(\w+)/ig;
            var parames = {};
            obj['action'].replace(pattern, function (a, b, c) {
                parames[b] = c;
            });
            for (par in parames)
                form.append("<input type='hidden' name='" + par + "' value='" + parames[par] + "'>");
        }
    ...
    if (confirm_flag) {
        var select_id = "id";
        if (obj['select_id'] != undefined)
            select_id = obj['select_id'];
        art.dialog.confirm('你确认删除操作？', function () {
            if ($("input[name='" + select_id + "[]']:checked").size() > 0)
                form.submit();
            else
                art.dialog.tips("<p class='warning'>没有选择任何项目，无法删除</p>");
        });
    } else {
        form.submit();
    }
...
(function ($) {
    $.fn.Condition = function (o) {
        ...
        if (fields_select != '')
            fields_select = "<select><option value='noselect'>添加筛选字段</option>" + fields_select + "</select>";
        ...
        $("#condition_create").on("click", function () {
            ...
            $("#condition_table tr:has(td)").each(function (i) {
                ...
                if (item.eq(3).val() != '' && con.indexOf(str) == -1)
                    con += str;
            });
    ...
    $(".operat").each(function () {
        ...
        operat.find(".action").on("mouseenter DOMNodeInserted", function () {
            ...
            if (offset.top + operat.find(".menu_select").height() + height < $(window).height()) {
                if (offset.left + menu_select_width < $(window).outerWidth())
                    operat.find(".menu_select").offset({left: offset.left, top: (Math.floor(offset.top) + 1 + height)});
                else
                    operat.find(".menu_select").offset({left: (offset.left + action_width - menu_select_width), top: (Math.floor(offset.top) + 1 + height)});
                operat.find(".action").removeClass("top").addClass("bottom");
            } else {
                if (offset.left + menu_select_width < $(window).outerWidth())
                    operat.find(".menu_select").offset({left: offset.left, top: Math.ceil(offset.top) - 1 - operat.find(".menu_select").height()});
                else
                    operat.find(".menu_select").offset({left: (offset.left + action_width - menu_select_width), top: Math.ceil(offset.top) - 1 - operat.find(".menu_select").height()});
        ...
        function rander(data) {
            ...
            if (typeof data == "object") {
                ...
                for (var i in data) {
                    data[i]['odd-even'] = "odd";
                    if (num++ % 2 == 0)
                        data[i]['odd-even'] = "even";
                    str += o.content.replace(/{\s*([^}]+)\s*}/ig, function (s0, s1) {
                        var tem = s1.split("|");
                        if(s1=='photos'&&data[i]['photos'].length!=0){
                            var img_str = "";
                            for(var j in data[i]['photos']){
                                img_str+='<img src="http://img.buy-d.cn'+data[i]['photos'][j]+'!/fwfh/720x720" onclick="showImg(this);">';
                            }
                            return img_str;
                        }
                        if (tem.length == 2) {
...
(function ($) {
    $.fn.countdown = function (options) {
        ...
        var prezero = function (s, l) { //补前导零
            s = s.toString();
            for (i = 0; i < l - s.length; i++)
                s = 0 + s;
            return s;
        };
        var update = function (obj, timestamp, offsetseconds, local) { //本地时间,对象,时间戳,偏移时间,是否使用本地时间
            offsetseconds = offsetseconds != undefined ? parseInt(offsetseconds) : 0;
            local = local != undefined ? local : true;
            if (timestamp) {
                var serverTime = parseFloat(timestamp + "000");
                var timeDiff = serverTime - localTime;
                if (local) {
                    var d = new Date(+Date.now() + timeDiff + (offsetseconds * 1000));
                } else {
                    var d = new Date();
                    var utc = d.getTime() + (d.getTimezoneOffset() * 60000);
                    d = new Date(utc + timeDiff + (offsetseconds * 1000));
                }
            } else {
                var d = new Date();
            }
            ...
            setTimeout(function () {
                SG.time.update(obj, timestamp, offsetseconds, local);
            }, 1000);
        };


        var self = this;
        var defaults = {id: 'countdown', remainseconds: 86400000, format: '{d}天 {h}小时{m}分{s}.{mi}秒', callback: function () {}};
        var o = $.extend(defaults, options);

        function runTime() {
            o.remainseconds = o.remainseconds - 100;
            var time = o.remainseconds;
            ...
            var str = o.format.replace(/{\s*([^}]+)\s*}/ig, function (s0, s1) {
                return date[s1];
            });
            self.html(str);
            if (leftsecond > 0)
...
function setCenter(mainElement, element)
{
    var windowScreen = document.documentElement;

    var main_left = (windowScreen.clientWidth - mainElement.clientWidth) / 2 + "px";
    var main_top = (windowScreen.clientHeight - mainElement.clientHeight) / 2 + "px";
    mainElement.style.left = main_left;
    mainElement.style.top = main_top;

    var content_left = (mainElement.clientWidth - element.clientWidth) / 2 + "px";
    var content_top = (mainElement.clientHeight - element.clientHeight) / 2 + "px";
    element.style.left = content_left;
    element.style.top = content_top;
}
...
$(function () {
    ...
    $("#search-keyword").on("focus", function () {
        $("#search-panel").show();
        $(".header-r").hide();
        $(".header-l a:first").hide();
        $(".header-l a:last").show();
        $(".header-c").css("margin", "0 0 0 0");
        var recentlysearch = $.parseJSON(localStorage.getItem("recentlysearch"));
        recentlysearch = recentlysearch ? recentlysearch : [];
        var unique_arr = new Array();
        for(var i in recentlysearch){
            if($.inArray(recentlysearch[i],unique_arr)==-1){
                unique_arr.push(recentlysearch[i]);
            }
        }
        $(unique_arr).each(function (i, j) {
            $("#recentlywordsBox").prepend("<a href='javascript:;'>" + j + "</a>");
        });
    }).on("blur", function () {
        setTimeout(function () {
            $("#search-panel").hide();
            $(".header-l a:first").show();
            $(".header-l a:last").hide();
            $(".header-r").show();
            $(".header-c").css("margin", "0 48px 0 85px");
        }, 100);
    });
    ...
    $("#search-form").submit(function () {
        var recentlysearch = $.parseJSON(localStorage.getItem("recentlysearch"));
        recentlysearch = recentlysearch ? recentlysearch : [];
        recentlysearch.push($("#search-keyword").val());
