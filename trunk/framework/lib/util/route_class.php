...
class Route {
    ...
    public function __construct($rules = array()) {
        if (empty(self::$routes) && is_array($rules)) {
            foreach ($rules as $pattern => $value) {
                ...
                if (!isset(self::$routes[$routes_key]))
                    self::$routes[$routes_key] = $value;
                ...
                if (!isset(self::$rules[$value]))
                    self::$rules[$value] = $rules_key . self::$splitChars . $routes_key;
                else
                    self::$rules[$value] = array_merge((is_string(self::$rules[$value]) ? array(self::$rules[$value]) : self::$rules[$value]), array($rules_key . self::$splitChars . $routes_key));
            }
        }
    }
        ...
        foreach (self::$routes as $pattern => $value) {
            if (preg_match('/^' . str_replace('/', '\/', $pattern) . '$/', $url, $matches)) {
                ...
                if (!isset($matches['con']))
                    $matches['con'] = (isset($ca[0]) && $ca[0] != '<_c>') ? $ca[0] : 'index';
                if (!isset($matches['act']))
                    $matches['act'] = (isset($ca[1]) && $ca[1] != '<_a>') ? $ca[1] : 'index';
                return $matches;
            }
        }
    }
    ...
    public function createRoute($params, $param_key = '') {
        if (is_array($params)) {
            ...
            $keys = array();
            if (isset($rules[$_c . '/' . $_a . $param_key]))
                $keys[] = $_c . '/' . $_a . $param_key;
            if (isset($rules['<_c>/' . $_a . $param_key]))
                $keys[] = '<_c>/' . $_a . $param_key;
            if (isset($rules[$_c . '/<_a>' . $param_key]))
                $keys[] = $_c . '/<_a>' . $param_key;
