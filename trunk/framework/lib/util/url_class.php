...
class Url {
    ...
    static function pathinfo() {
        ...
        if (null === ($request_uri = self::requestUri()))
            return '';
    ...
    public static function getRefRoute() {
        if (isset($_SERVER['HTTP_REFERER']) && ($_SERVER['HTTP_REFERER'] & self::getEntryUrl()) == self::getEntryUrl())
    ...
    static function urlFormat($path) {
        ...
        switch (substr($path, 0, 1)) {
            ...
            case '@': {
                    $fullpath = self::baseDir() . substr($path, 1);
                    if (substr(ltrim($fullpath, "/"), 0, 12) == "data/uploads") {
                        $upyun = Config::getInstance()->get("upyun");
                        if ($upyun) {
                            $fullpath = $upyun['upyun_cdnurl'] . '/' . ltrim($fullpath, "/");
                        }
                    }
                    return $fullpath; //解析成绝对路径
                }
            case '#': //解析成主题的绝对地址
                {
    ...
    public static function createUrl($_url) {
        ...
        if (self::getUrlFormat() == 'get' || $routes !== null) {
            ...
            if (self::getUrlFormat() == 'get')
                $url = '?' . http_build_query($param_arr); //.($param==''?'':'&'.$param);
            else {
    ...
    public static function setUrlFormat($value) {
        if ($value == 'get' || $value == 'path')
            self::$_urlFormat = $value;
    }
