...
class Image {
    ...
    public function thumb($filename, $width = 100, $height = 100, $quality = 95) {
        if (preg_match('@http://@i', $filename))
            return $filename;
        if(!is_file($filename)){
            return str_replace(APP_ROOT, '', $filename);
        }
        ...
        if ($this->suffix == 'w_h')
            $thumb = '__' . $width . '_' . $height;
        else if ($this->suffix == 'f_w_h')
            $thumb = '__' . $width . '_' . $height;
        else if ($this->suffix == 'f_w_h.jpg')
            $thumb = '__' . $width . '_' . $height;
        else if ($this->suffix == '')
            $thumb = '';
        else
        ...
        if ($this->suffix == 'f_w_h')
            $tofile = $this->thumbPath . $filename . $thumb . $ext;
        else if ($this->suffix == 'f_w_h.jpg')
            $tofile = $this->thumbPath . $filename . $thumb . '.jpg';
        else
            $tofile = $this->thumbPath . substr($filename, 0, $end) . $thumb . $ext;

        switch ($type) {
            case 1: imagegif($imBox, $tofile, $quality);
                break;
            case 2: imagejpeg($imBox, $tofile, $quality);
                break;
            case 3: {
    ...
    public function cut($dst_image, $src_image, $src_x, $src_y, $src_w, $src_h, $dst_w = 100, $dst_h = 100, $quality = 90) {
        ...
        switch ($type) {
            case 1: imagegif($dst_r, $dst_image, $quality);
                break;
            case 2: imagejpeg($dst_r, $dst_image, $quality);
                break;
            case 3: {
                    $quality = $quality > 9 ? 9 : $quality;
    ...
    public function waterMark($sourcefile, $position = 0) {
        ...
        switch ($imgtype) {//取得背景图片的格式
            case 1:imagegif($sourceImage, $sourcefile);
                break;
            case 2:imagejpeg($sourceImage, $sourcefile);
                break;
            case 3:imagepng($sourceImage, $sourcefile);
                break;
    ...
    private function createImage($type, $filename) {
        //1 = GIF，2 = JPG，3 = PNG
        switch ($type) {
            case 1: $im = imageCreateFromGif($filename);
                break;
            case 2: $im = imageCreateFromJpeg($filename);
                break;
            case 3: $im = imageCreateFromPng($filename);
                break;
            default: {
