        ...
        if (!is_null($this->files)) {
            if (is_array($this->files['name'])) {
                ...
                for ($i = 0; $i < $num; $i++) {
                    if ($this->checkSize($this->files['size'][$i]) && $this->checkType($this->files['name'][$i])) {
                        ...
                        if (!file_exists(dirname($realpath)))
                            $this->mkdir($this->getUploadPath() . dirname($realpath));
                        move_uploaded_file($this->files['tmp_name'][$i], $this->getUploadPath() . $realpath);
                        $this->info[] = array('name' => $this->files['name'][$i], 'path' => $realpath, 'size' => $this->files['size'][$i], 'status' => self::SUCCESS, 'msg' => '上传成功');
                    }
                    else if (!$this->checkSize($this->files['size'][$i])) {
                        $this->info[] = array('name' => $this->files['name'][$i], 'path' => '', 'size' => $this->files['size'][$i], 'status' => self::FAILSIZE, 'msg' => '超出了文件规定的大小');
                    } else if (!$this->checkType($this->files['name'][$i])) {
                        $this->info[] = array('name' => $this->files['name'][$i], 'path' => '', 'size' => $this->files['size'][$i], 'status' => self::FAILTYPE, 'msg' => '文件上传的类型不符合');
                    }
                }
                ...
                if ($this->checkSize($this->files['size']) && $this->checkType($this->files['name'])) {
                    ...
                    if (!file_exists(dirname($realpath)))
                        $this->mkdir($this->getUploadPath() . dirname($realpath));
                    $this->info[] = array('name' => $this->files['name'], 'path' => $realpath, 'size' => $this->files['size'], 'status' => self::SUCCESS, 'msg' => '上传成功');
                    move_uploaded_file($this->files['tmp_name'], $this->getUploadPath() . $realpath);
                }
                else if (!$this->checkSize($this->files['size'])) {
                    $this->info[] = array('name' => $this->files['name'], 'path' => '', 'size' => $this->files['size'], 'status' => self::FAILSIZE, 'msg' => '超出了文件规定的大小');
                } else if (!$this->checkType($this->files['name'])) {
                    $this->info[] = array('name' => $this->files['name'], 'path' => '', 'size' => $this->files['size'], 'status' => self::FAILTYPE, 'msg' => '文件上传的类型不符合');
                }
            }
        }
    }
