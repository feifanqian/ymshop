...
//应用开发路径
define('APP_CODE_ROOT', APP_ROOT . 'protected' . DIRECTORY_SEPARATOR);
//调式开关
define('DEBUG', true);
//DEBUG TRACE开关
...
class Tiny {
    ...
    public static function setPath($name, $path) {
        if (empty($path))
            unset(self::$_paths[$name]);
        else
            self::$_paths[$name] = rtrim($path, '\/') . '/';
    }
    ...
    public static function initSystemHandler() {
        ...
        if (EXCEPTION_HANDLER)
            set_exception_handler(array('Tiny', 'handleException'));
        if (ERROR_HANDLER)
            set_error_handler(array('Tiny', 'handleError'), error_reporting());
    }
    ...
    public static function handleError($code, $message, $file, $line, $errContext) {
        ...
        try {
            $errorType;
            switch ($code) {
                case E_ERROR:$errorType = 'ERROR';
                    break;
                case E_WARNING:$errorType = 'WARNING';
                    break;
                case E_NOTICE:$errorType = 'NOTICE';
                    break;
                case E_USER_ERROR:$errorType = 'USER_ERROR';
                    break;
                case E_USER_WARNING:$errorType = 'USER_WARNING';
                    break;
                case E_USER_NOTICE:$errorType = 'USER_NOTICE';
                    break;
                case E_PARSE:$errorType = 'PARSE_ERROR';
                    break;
    ...
    public static function autoload($className) {
        if (preg_match('/^[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*$/', $className) == 0)
            return null;
        if (isset(self::$_coreClasses[$className])) {
            ...
            if (isset(self::$_classes[$className])) {
                ...
                } else if (strrchr($className, 'Action') == 'Action') {
                    $fileName = APP_CODE_ROOT . 'actions/' . strtolower(substr($className, 0, -6)) . '.php';
                    if (is_file($fileName)) {
                        include($fileName);
                        return true;
                    }
    ...
    public static function loadExtendsClass($className) {
        if (isset(self::$_classes_index[$className])) {
            if (is_file(self::$_classes_index[$className]))
                include(self::$_classes_index[$className]);
            return true;
        }else {
            foreach (self::$_classes as $classPath) {
                ...
                if (is_file($class_index)) {
                    ...
                    foreach ($classes_index as $k => $v) {
                        if (!isset(self::$_classes_index[$k]))
                            self::$_classes_index[$k] = $class_base_path . $v;
                    }
                }
                if (isset(self::$_classes_index[$className])) {
                    if (is_file(self::$_classes_index[$className]))
