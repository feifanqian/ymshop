...
class Controller extends Object {
    ...
    public function run() {
        ...
        else if ($this->module->popRequestStack($this->id . '@' . $id)) {
            throw new Exception("Can't repeat redirect to the same position, you action is {$this->id}.", E_USER_ERROR);
        }
        //在创建Action的时候进行token验证
    ...
    public function getViewFile($viewName) {
        if (($theme = Tiny::app()->getTheme()) !== null && ($viewFile = $theme->getViewFile($this, $viewName)) !== null) {
            if (is_file($viewFile . $this->templateExt))
                return $viewFile;
        }
    ...
    public function getLayoutFile() {
        ...
        if (($theme = Tiny::app()->getTheme()) !== null && ($layoutFile = $theme->getLayoutFile($this, $layoutName)) !== null) {
            if (file_exists($layoutFile . $this->templateExt))
                return $layoutFile;
        }
    ...
    public function checkRight($actionId) {
        ...
        if ($isCheckRight) {
            ...
            if (isset($manager['roles'])) {
                ...
                $this->redirect(Url::fullUrlFormat("/admin/login"));
                return false;
            }
        }
        return true;
    ...
    public function renderExecute($__runfile0123456789, $__data0123456789) {
        ...
        if (is_array($__data0123456789))
            $__datas0123456789 += $__data0123456789;
        if (is_array($this->datas))
            $__datas0123456789 += $this->datas;
    ...
    public function createAction($id) {
        ...
        if ($this->checkRight($id) == false) {
            var_dump($this->defaultAction);die;
            $this->noRight();
        } else {
            //如果控制器直接定义了方式
