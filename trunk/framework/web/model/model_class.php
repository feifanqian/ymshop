...
class Model {
    ...
    public function join($str) {
        ...
        if (strpos($str, 'join') === false)
            $this->sql['join'] = 'left join ' . preg_replace('/(\w+)(?=\s+as\s+\w+(,|\)|\s))/i', "tiny_$1 ", $str);
        else
    ...
    private function formatField($typeInfo, $value) {
        ...
        if (isset($len)) {
            if (!is_array($value) && TString::strlen($value) > $len)
                $value = TString::msubstr($value, 0, $len, 'utf-8', '');
        }
        if (is_array($value))
    ...
    private function changeWhere() {
        ...
        if ($where != '') {
            if ($this->sql['where'] != '')
                $this->sql['where'] = $this->sql['where'] . ' and ' . $where;
            else
